...
class Crawler{
  ...
  constructor(config, seedUrls){
    ...
    class UrlQueueClass {
      ...
      enqueue(run, context) {
        if(priorityRegExp && (priorityRegExp.test(_.get(context, 'new.url')) || priorityRegExp.test(_.get(context, 'new.text')) || priorityRegExp.test(_.get(context, 'parent.metas.title')))){
    ...
    function getInfoCount(){
      return {
        done: Object.keys(doneUrls).length,
        todo: Object.keys(todoUrls).length,
        queue: queue.queue._queue.length,
        lowPriorityQueue: queue.queue._lowPriorityQueue.length
      };
    }
    ...
    function crawl(urlData){
      ...
      .then(([newUrls, response, metas]) => {
        ...
        self.emitter.emit('progress', getInfoCount());
  ...
  extractUrls(currentUrl){
    return Request({ // TODO this should be overridable by config
      headers: {
