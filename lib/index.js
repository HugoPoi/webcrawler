...
class Crawler{
  ...
  constructor(config, seedUrls){
    ...
    class UrlQueueClass {
      ...
      enqueue(run, urlData) {
        if(priorityRegExp && priorityRegExp.test(urlData.url)){
    ...
    function crawl(urlData){
      ...
      .then(([newUrls, response, metas]) => {
        ...
        if(config.nofollow && urlData.metas && /nofollow|noindex/i.test(urlData.metas.robots)){
          return;
        }
        ...
        .uniq()
        .filter(url => {
          return !doneUrls[url] && !todoUrls[url];
        })
        .map(url => {
          return { url: url };
        })
    ...
    this.promise = queue.onIdle().then(() => doneUrls);
  ...
  extractUrls(currentUrl){
    ...
    .then(response => {
      ...
      if (response.statusCode === 200) {
        ...
        metas.robots = $('meta[name="robots"]').attr('content');
        ...
        console.error('Response code %d for %s', response.statusCode, currentUrl);
      }
