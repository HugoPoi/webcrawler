...
class Crawler{
  ...
  constructor(config, seedUrls){
    ...
    this.queue = new PQueue({ concurrency: config.concurrency || 20, queueClass: UrlQueueClass });
  ...
  crawl(urlData){
    ...
    return this.extractUrls(urlData.url)
      .then(([newUrls, response, metas]) => {
        ...
        if(this.config.nofollow && urlData.metas && /nofollow/i.test(urlData.metas.robots)){
          return;
        }
        if(this.config.noindex && urlData.metas && /noindex/i.test(urlData.metas.robots)){
          return;
        }
        ...
        let urlsToMerge = _.chain(newUrls)
          .filter(newUrlObj => {// TODO maybe this can be overide by config ?
            ...
            if(!this.config.includeSubdomain){
              return parsedUrl.hostname === this.config.hostname && !parsedUrl.pathname.endsWiths(['.jpg', '.png', '.pdf', '.mp4', '.mp3', '.zip', '.gif', '.rar']);
            }else{
              return Tldjs.getDomain(parsedUrl.hostname) === Tldjs.getDomain(this.config.hostname) && !parsedUrl.pathname.endsWiths(['.jpg', '.png', '.pdf', '.mp4', '.mp3', '.zip', '.gif', '.rar']);
            }
          })
          .uniqBy('url')
          .filter(newUrlObj => {
  ...
  start(){
    if(this.promise){
      ...
      this.promise = this.queue.onIdle().then(() => {
